    ...
    function oa(f, ia, m) {
        ...
        function ca() {
            ...
            if (D) {
                ...
                k.length = 0;
                var Da = da(r, c.horizontal ? "paddingLeft" : "paddingTop"),
                    ia = da(r, c.horizontal ? "paddingRight" : "paddingBottom"),
                    Ea = da(A, c.horizontal ? "marginLeft" : "marginTop"),
                    a = da(A.slice(-1), c.horizontal ? "marginRight" : "marginBottom"),
                    l = 0,
                    q = "none" !== A.css("float"),
                    a = Ea ? 0 : a;
                n = 0;
                A.each(function(a, b) {
                    var j = h(b),
                        d = j[c.horizontal ? "outerWidth" : "outerHeight"](!0),
                        K = da(j, c.horizontal ? "marginLeft" : "marginTop"),
                        j = da(j, c.horizontal ? "marginRight" : "marginBottom"),
                        e = {
                            el: b,
                            size: d,
                            half: d / 2,
            ...
            if (!F && 0 < s) {
                ...
                if (D) h.each(k, function(a,
                    ...
                    for (; m - s < e.end;) v.push(m), m += s; if (W[0] && b !== v.length) {
        ...
        function va(a) {
            ...
            if (D) {
                for (var d = c = b = !1, f = 0, g = k.length; f < g; f++)
                    if (!1 === b && a <= k[f].start + k[f].half && (b = f), !1 === d && a <= k[f].center + k[f].half && (d = f), f === g - 1 || a <= k[f].end + k[f].half) {
                        c = f;
                        break
                    }
                j.firstItem =
        ...
        function Ja(a, b) {
            for (var c = 0, d = E[a].length; c < d; c++)
                if (E[a][c] === b) return c;
            return -1
        }
        ...
        function Xa(a) {
            if (c.keyboardNavBy) switch (a.which) {
                ...
                case c.horizontal ? 39:
                    40 : L(a), d["pages" === c.keyboardNavBy ? "nextPage" : "next"]()
            }
        }
        ...
        ia = "basic" === c.itemNav;
        var J = "forceCentered" === c.itemNav,
            P = "centered" === c.itemNav || J,
            D = !F && (ia || P || J),
            Ta = c.scrollSource ?
                h(c.scrollSource) : C,
            ab = c.dragSource ? h(c.dragSource) : C,
            Z = h(c.forward),
            ea = h(c.backward),
            $ = h(c.prev),
            aa = h(c.next),
        ...
        d.slideBy = function(a, b) {
            if (D) d[P ? "toCenter" : "toStart"](w((P ? g.centerItem : g.firstItem) + c.scrollBy * a, 0, k.length));
            else G(e.dest + a, b)
        };
        ...
        d.off = function(a, b) {
            if (b instanceof Array)
                ...
                for (var c = a.split(" "), e = 0, f = c.length; e < f; e++)
                    if (E[c[e]] = E[c[e]] || [], "undefined" === Q(b)) E[c[e]].length = 0;
                    else {
                        var g = Ja(c[e], b); - 1 !== g && E[c[e]].splice(g, 1)
                    }
        };
        d.destroy = function() {
        ...
        d.init = function() {
            if (!d.initialized) {
                ...
                H ? la && a.css(H, la) : ("static" === I.css("position") && I.css("position", "relative"), a.css({
                    position: "absolute"
                }));
