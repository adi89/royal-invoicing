...
describe ContactsController do
  ...
  describe 'GET Index' do
    it "has a 200 status code" do
      ...
      get :index, {group_id: @contact.user.group.id, use_route: "group_contacts"}
  ...
  describe 'Show page' do
    it 'renders gives us the show page' do
      login_user(@contact.user)
      get :show, {group_id: @contact.user.group.id, id: @contact.id, use_route: "group_contact"}
      expect(response.status).to eq(200)
  ...
  describe 'New invoice' do
    it 'renders the new page and has a 200 status ' do
      login_user(@contact.user)
      get :new, {group_id: @contact.user.group.id, use_route: "new_contact_invoice"}
      expect(response.status).to eq (200)
  ...
  describe "sort" do
    it 'sorts the contacts successfully passing the sort action' do
      login_user(@contact.user)
      xhr :get, :sort, {group_id: @contact.user.group.id, use_route: "sort_contact_invoices",
                        "type"=>"due_date",
                        "forward" => "false",
                        "ids" => "#{@contact.id}}"
                        }
  ...
  describe 'company_ajax' do
    it 'does the company ajax action' do
      login_user(@contact.user)
      xhr :post, :company_ajax, {use_route: "company_ajax",
  ...
  describe 'update' do
    it 'successfully updates the contact info in update action' do
      login_user(@contact.user)
      patch :update, {use_route: "group_contact", :group_id => @contact.user.group.id, :id => @contact.id, "contact" =>
                      {"name"=>"Adi Singh",
  ...
  describe 'ajax create' do
    it 'it follows contact action logic and redirects' do
      login_user(@contact.user)
      xhr :post, :create, {use_route: :group_contacts, :group_id => @contact.user.group.id, "contact" =>
                  {"name"=>"blah ",
